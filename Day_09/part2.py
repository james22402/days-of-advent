...
def find_bad_num_sum(codes, bad_num, i):
    badsum = 0
    j = i
    while badsum < bad_num:
        badsum += codes[j]
        if badsum == bad_num:
            return (codes[i],codes[j])
        j += 1
    return find_bad_num_sum(codes=codes,bad_num=bad_num, i=i+1)
    


print(find_bad_num_sum(codes=codes,bad_num=find_bad_number(preamble=header,code_list=codes),i=0))